...
"""
diffmean_holdout_lambda_sweep_fixed.py
...
This version is adapted to the refactored LLaDA `generate()` API:
- it uses the base LLaDA commit policy via `fill_strategy`
- it does NOT pass remasking arguments when remasking is disabled
- it keeps steering orthogonal to generation policy
- it evaluates ONLY the `real_full_pooled` variant by default
- it writes to a new directory, `diffmean_holdout_lambda_sweep_fixed`, so
  previous sweep outputs are preserved

What this does
--------------
- loads prompts from amazon_polarity train[50000:50500]
- evaluates only the activation variant:
    * real_full_pooled
- sweeps DiffMean steering strength lambda
...
Example:
    python diffmean_holdout_lambda_sweep_fixed.py \
...
VARIANTS = ["real_full_pooled"]

EVAL_START_IDX = 50_000
EVAL_END_IDX = 50_200

# Base LLaDA generation setup.
# `fill_strategy="low_confidence"` means:
#   - predict candidate tokens for all currently masked positions
#   - score each candidate by its model confidence / probability
#   - commit the highest-confidence masked tokens first at each step
# This is the standard confidence-based LLaDA commit policy.
GEN_PARAMS = dict(
    temperature=0.0,
    steps=30,
    gen_length=30,
...
def run_lambda_sweep_for_variant(
    ...
    for lam in lambdas:
        ...
        metrics["fill_strategy"] = GEN_PARAMS["fill_strategy"]
        _write_json(variant_eval_dir / f"{tag}_metrics.json", metrics)
        results[tag] = metrics

    csv_lines = [
        "variant,lambda,n_total,n_valid,n_invalid,invalid_fraction,bad_pattern_fraction,repetition_fraction,empty_fraction,sent_answer_mean,sent_answer_fraction,sent_combined_mean,ppl_answer_mean,ppl_combined_mean,mean_answer_words_all,mean_answer_words_valid_only,fill_strategy"
    ]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=VARIANTS,
        help="Variants to evaluate. Defaults to real_full_pooled only.",
    )
    parser.add_argument(
        "--lambda-min",
        type=int,
    ...
    parser.add_argument(
        "--fill-strategy",
        type=str,
        default=GEN_PARAMS["fill_strategy"],
        choices=["low_confidence", "random"],
        help="Base LLaDA commit policy to use during generation.",
    )
...
def main() -> None:
    ...
    GEN_PARAMS["fill_strategy"] = args.fill_strategy

    out_root = args.out_root
    probe_root = out_root / "probes"
    eval_root = out_root / "diffmean_holdout_lambda_sweep_fixed"
    ...
    for variant in args.variants:
        ...
        for _, m in variant_results.items():
