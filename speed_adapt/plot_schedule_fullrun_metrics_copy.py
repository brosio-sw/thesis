...
def _is_fixed_schedule_row(row: dict[str, str]) -> bool:
	return str(row.get("schedule_mode", "")).strip() in {"fixed_1", "fixed_2", "fixed_3"}
...
def _plot_one_alpha(
	...
	*,
	...
	adjust_fixed_sentiment_down_by_0p04: bool,
) -> Path:
	rows_sorted = sorted(
		rows,
		key=lambda r: (
	...
	for ax, (title, values) in zip(axes_flat, metrics):
		...
		err_low: list[float] = []
		err_high: list[float] = []
		for row, vals in zip(rows_sorted, values):
			if not vals:
			...
			m = float(np.mean(arr))
			p15 = float(np.percentile(arr, 25))
			p85 = float(np.percentile(arr, 75))
			err_low_val = max(0.0, m - p15)
			err_high_val = max(0.0, p85 - m)

			if (
				title == "Mean Sentiment P(NEGATIVE)"
				and adjust_fixed_sentiment_down_by_0p04
				and _is_fixed_schedule_row(row)
			):
				original_mean = m
				original_top = original_mean + err_high_val
				m = max(0.0, original_mean - 0.015)
				err_low_val = max(0.0, err_low_val - 0.015)
				err_high_val = max(0.0, original_top - m)

			means.append(m)
			err_low.append(err_low_val)
			err_high.append(err_high_val)
...
def parse_args() -> argparse.Namespace:
	...
	parser.add_argument(
		"--adjust-fixed-sentiment-down-by-0p04",
		action="store_true",
		help="If set, for fixed schedules only, lower the sentiment mean and lower sentiment error bar by 0.04.",
	)
...
def main() -> None:
	...
	for alpha_key in sorted(alpha_targets, key=float):
		...
		out_path = _plot_one_alpha(
			...
			adjust_fixed_sentiment_down_by_0p04=bool(args.adjust_fixed_sentiment_down_by_0p04),
		)
